...
function translateNumberSource(number) {
  ...
  number = (0, utils_1.formatNumberBeforeTranslation)(number)
  if (number.includes(".")) {
  ...
  for (let classIndex = 0; ; classIndex++, number = (0, utils_1.removeLastNSymbols)(number, 3)) {
    if (number.length < 4) {
...
function translateNonIntegerPart(number) {
  ...
  return (
    (nonIntegerPartType === types_1.NonIntegersPostfixesEnum.ENDS_WITH_ONE
      ? (0, utils_1.removeLastNSymbols)(translated, 4) + "одна"
      : translated) +
